...
# 页面内容设置
# 页面名称
st.set_page_config(page_title="Adverse events", layout="wide")
# 标题
st.title('The machine-learning model to predict major adverse events for heart failure')

st.markdown('_This is a webApp to predict the risk of major adverse events (**death, respiratory failure and MODS**) \
...
LYM= st.sidebar.number_input(label='lYM', min_value=0.1,
                              max_value=100.00,
                              value=10.0,
                              step=0.1)
...
Cl = st.sidebar.number_input(label='Cl', min_value=0.0,
                            max_value=200.0,
                            value=0.0,
                            step=0.1)
